...
/*
    ...
    Explanation: The subsequence [7,7] has the sum of its elements equal to 14 which is not strictly
     greater than the sum of elements not included (14 = 4 + 4 + 6). Therefore, the subsequence [7,6,7] is the
...
public class MinimumSubsequenceInNonIncreasingOrder {
    ...
    public static void main(String[] args) {
        ...
        AtomicInteger sum = new AtomicInteger(Arrays.stream(nums).sum());
        AtomicInteger currSum = new AtomicInteger();

        Arrays.stream(nums)
                .boxed()
                .sorted(Collections.reverseOrder())
                .filter( i -> {
                    if(currSum.get() > sum.get()){
                        return false;
                    }else {
                        sum.set(sum.get() - i);
                        currSum.set(currSum.get() + i);
                    }
                    return true;
                })
